    ...
    <nav className="flex flex-wrap justify-between items-center">
      ...
      <div className={`${!isMobileMenuShown && "hidden"} w-full lg:w-auto lg:block`}>
        <ul className="lg:space-x-8 flex flex-col lg:flex-row bg-gray-50 lg:bg-transparent text-lg border border-gray-100 lg:border-none rounded-lg p-4">
          ...
          <li className="px-3 py-2 cursor-pointer hover:bg-gray-100">
            <Link to="/reservations" className="block">Mes Reservations</Link>
          </li>
          {/* Condition pour afficher le nom de l'utilisateur ou le lien de connexion */}
          {user?.userDetails
            ? <li className="px-3 py-2 cursor-pointer hover:bg-gray-100">{user.userDetails.lastname}</li>
            : <li className="px-3 py-2 cursor-pointer hover:bg-gray-100"><Link to="/login" className="block">Connexion</Link></li>}
        </ul>
      </div>
    </nav>
  );
